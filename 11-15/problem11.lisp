...
(defconstant +REF+ (flip (pa$ #'aref +DATA+)))


(defun down (x y)
  (if (> y 16) nil 
		  (loop 
			for i from y upto (+ y 3) 
			collect (list x i))))

(defun right (x y)
	(if (> x 16) nil
		  (loop 
			for i from x upto (+ x 3)
			collect (list i y))))

(defun diagonal-1 (x y)
  (if (or (> x 16) (> y 16)) nil
	(loop for i from x upto (+ x 3)
		  collect (list i (+ y (- i x))))))

(defun diagonal-2 (x y)
  (if (or (> 3 x) (> y 16)) nil
	(loop for i from x downto (- x 3)
		  collect (list i (+ y (- x i)))
	)
  ))

(defun prodps (point-lst)
 	(prod 
	  (mapcar 
		(lambda (p) 
	  	(apply +REF+ p)) point-lst)))



(defun maxim (x y)
  ;; (x,y) でもっとも大きい数
  (apply #'max (mapcar #'prodps (list (right x y) (down x y) (diagonal-1 x y) (diagonal-2 x y)))))


(print (apply #'max [(maxim x y) | x <- (range1-n 19 0) y <- (range1-n 19 0)]))
